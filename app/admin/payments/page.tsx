...
export default function AdminPaymentsPage() {
  ...
  async function save(request: Request) {
    ...
    const requestedStatus = patch.status ?? request.status
    const status = requestedStatus === 'confirmed' || requestedStatus === 'rejected' || requestedStatus === 'cancelled' || requestedStatus === 'proof_submitted'
      ? requestedStatus
      : hasInstructions ? 'pending' : requestedStatus
    ...
    const { data: updated, error } = await supabase.from('payment_requests').update({
      ...
      status,
      confirmed_by: status === 'confirmed' ? auth.user.id : null,
      confirmed_at: status === 'confirmed' ? new Date().toISOString() : null,
    }).eq('id', request.id).select('*').single()

    setSaving(null)
    if (error) {
      setMessage(`Could not save payment update: ${error.message}`)
      return
    }
    if (!updated) {
    ...
    setMessage(status === 'confirmed' ? 'Payment marked as confirmed.' : status === 'rejected' ? 'Payment rejected.' : 'Payment instructions saved and sent to the user.')
    await load()
  }
