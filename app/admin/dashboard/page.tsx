...
export default function AdminDashboard() {
  ...
  async function load() {
    ...
    const { data: isAdmin, error: adminError } = await supabase.rpc('is_admin', { uid: auth.user.id })
    if (adminError || !isAdmin) {
      await supabase.auth.signOut()
      setAuthorized(false)
      setSummary(null)
      setMessage(adminError?.message || 'Administrator access required.')
      setLoading(false)
      return
    }
    setAuthorized(true)
    ...
    if (userCountError || paymentCountError) setMessage([userCountError?.message, paymentCountError?.message].filter(Boolean).join(' · ') || 'Could not load totals.')
    setTotalUsers(userCount ?? 0)
    setTotalPayments(paymentCount ?? 0)
    setLoading(false)
  }
  ...
  useEffect(() => {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'SIGNED_OUT' || !session) {
        setAuthorized(false)
        setSummary(null)
        setMessage('Your administrator session has ended. Please sign in again.')
  ...
  if (!authorized || !summary) return <main className="payment-page container"><div className="payment-card"><h1>Administrator access required.</h1><p>{message || 'Please sign in with an administrator account.'}</p><a className="btn primary" href="/admin/login">Sign in again →</a></div></main>
