...
export default function PaymentsPage() {
  ...
  async function load() {
    ...
    const [{ data: cats }, { data: reqs }] = await Promise.all([
  ...
  async function submitRequest() {
    ...
    const { error } = await supabase.from('payment_requests').insert({
      ...
      base_amount_usd: selected.base_amount_usd,
      country_code: country.trim().toUpperCase(),
      currency_source: 'admin_set',
    })
  ...
  async function uploadProof(requestId: string) {
    ...
    const { error } = await supabase.from('payment_proofs').insert({ payment_request_id: requestId, user_id: userId, storage_path: path, original_filename: file.name, mime_type: file.type })
  ...
  return <main className="payment-page container">
    ...
    <div className="payment-header"><div><div className="eyebrow">Services & payments</div><h1>Request a service.</h1><p>Choose what you need. The platform keeps a reference value in USD, while the administrator supplies the applicable local currency and payment account for your country.</p></div><a className="btn secondary" href="/admin/payments">Admin payment desk</a></div>
    {message && <div className="info-box"><strong>Payment update</strong><p>{message}</p></div>}
    <section className="payment-layout">
      <div className="payment-card">
        <h2>New service request</h2>
        <label>Service<select value={categoryId} onChange={(e) => setCategoryId(e.target.value)}>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
        {selected && <div className="price-box"><strong>{selected.pricing_type === 'fixed' ? `$${Number(selected.base_amount_usd).toLocaleString()} USD reference` : selected.pricing_type === 'commission' ? `${selected.commission_min_percent}%–${selected.commission_max_percent}% commission` : 'Admin-set amount'}</strong><span>{selected.description}</span></div>}
        {needsAmount && <label>Requested amount / earnings basis<input type="number" min="0" value={variableAmount} onChange={(e) => setVariableAmount(e.target.value)} placeholder="Enter amount" /></label>}
        <label>Your country<input value={country} onChange={(e) => setCountry(e.target.value)} placeholder="e.g. Nigeria, United Kingdom, Canada" /></label>
        <label>Reason / notes <small>Optional</small><textarea rows={4} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Tell the admin what you need this service for." /></label>
        <button className="btn primary" onClick={submitRequest}>Request payment instructions →</button>
      </div>
      <div className="payment-card"><h2>My requests</h2>{requests.length === 0 ? <p className="muted">No payment requests yet.</p> : requests.map((r) => <article className="request-item" key={r.id}><div><strong>{r.title}</strong><span className="status">{r.status}</span></div><p>{r.local_amount ? `${r.currency} ${Number(r.local_amount).toLocaleString()}` : `${r.currency} ${Number(r.amount).toLocaleString()} reference`}</p>{r.payment_account_name && <div className="payment-instructions"><strong>Payment account</strong><p>{r.payment_account_name}{r.payment_account_number ? ` · ${r.payment_account_number}` : ''}</p>{r.payment_method && <p>Method: {r.payment_method}</p>}{r.payment_instructions && <p>{r.payment_instructions}</p>}{r.admin_payment_reference && <p>Reference: {r.admin_payment_reference}</p>}</div>}{['pending','rejected','cancelled'].includes(r.status) ? null : <div className="proof-upload"><input type="file" accept="image/*,.pdf" onChange={(e) => setProofFiles((current) => ({ ...current, [r.id]: e.target.files?.[0] ?? null }))} /><button className="btn secondary" disabled={!proofFiles[r.id]} onClick={() => uploadProof(r.id)}>Submit proof</button></div>}</article>)}</div>
    </section>
  </main>
}
