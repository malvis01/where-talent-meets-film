...
type Production = { user_id: string; company_name: string | null }
...
export default function PaymentsPage() {
  ...
  async function load(showLoading = true) {
    ...
    const [{ data: cats, error: categoryError }, { data: reqs, error: requestError }, { data: prods, error: productionError }] = await Promise.all([
      ...
      supabase.from('production_profiles').select('user_id,company_name').order('company_name'),
    ])
    if (categoryError || requestError || productionError) setMessage(categoryError?.message || requestError?.message || productionError?.message || 'Could not load payment data.')
    setCategories(cats ?? [])
    setProductions(prods ?? [])
    setRequests(reqs ?? [])
    if (!categoryId && cats?.[0]) setCategoryId(cats[0].id)
    if (!productionId && prods?.[0]) setProductionId(prods[0].user_id)
    setLoading(false)
  }
  ...
  async function submitRequest() {
    if (!userId || !selected || !productionId) { setMessage('Select the production company you are requesting payment from.'); return }
    ...
    const { error } = await supabase.from('payment_requests').insert({ user_id: userId, production_user_id: productionId, title: selected.name, reason: reason.trim() || null, amount, currency: 'USD', service_category_id: selected.id, base_amount_usd: selected.base_amount_usd, country_code: country.trim().toUpperCase(), currency_source: 'admin_set' })
