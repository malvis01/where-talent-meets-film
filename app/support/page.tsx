...
export default function SupportPage() {
  ...
  async function send(event: FormEvent) {
    ...
    if (!userId || !conversation || !body.trim() || sending) return
    setSending(true)
    setMessage('')
  ...
  return <main className="payment-page container"><a className="back-link" href="/account">← My account</a><div className="payment-header"><div><div className="eyebrow">Customer care</div><h1>Chat with admin.</h1><p>Ask about your account, services, payments, casting or anything you need help with.</p></div><span className="status">{conversation?.status === 'closed' ? 'Closed' : 'Open'}</span></div>{message && <div className="info-box"><p>{message}</p></div>}<section className="payment-card"><div className="support-thread">{messages.length === 0 ? <p className="muted">No messages yet. Send your first message below.</p> : messages.map((item) => <div className={`support-message ${item.sender_user_id === userId ? 'mine' : 'admin'}`} key={item.id}><div className="support-bubble"><strong>{item.sender_user_id === userId ? 'You' : 'Admin customer care'}</strong><p>{item.body}</p><small>{new Date(item.created_at).toLocaleString()}</small></div></div>)}</div><form onSubmit={send} className="support-compose"><textarea rows={4} value={body} onChange={(e) => setBody(e.target.value)} placeholder="Write your message to customer care…" required /><button className="btn primary" disabled={sending || !body.trim()}>{sending ? 'Sending…' : 'Send message →'}</button></form></section></main>
}
