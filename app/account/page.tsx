...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (error || !data.user) {
        if (active) {
          setMessage(error?.message || 'Please sign in to access your account.')
          setLoading(false)
        }
        return
      }
  ...
  return (
    <main className="payment-page container">
      ...
      <div className="payment-header">
        <div><div className="eyebrow">Your talent account</div><h1>Welcome{profile?.display_name ? `, ${profile.display_name}` : ''}.</h1><p>Your account is active. This is your platform home after sign-in.</p></div>
      ...
      <section className="payment-layout">
        ...
        <div className="payment-card"><h2>What&apos;s next</h2><p className="muted">Your login is now connected to the real Supabase account. Continue building your professional profile or access platform services.</p><div className="actions"><a className="btn primary" href="/actors/register">Build / update actor profile →</a><a className="btn secondary" href="/payments">Services & payments →</a></div></div>
      </section>
    </main>
  )
}
